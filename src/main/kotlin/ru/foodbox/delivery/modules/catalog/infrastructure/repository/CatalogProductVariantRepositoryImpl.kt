...
class CatalogProductVariantRepositoryImpl(
    ...
    override fun findAllActiveByProductId(productId: UUID): List<CatalogProductVariant> {
        return jpaRepository.findAllByProductIdAndIsActiveTrueOrderBySortOrderAscCreatedAtAsc(productId).map(::toDomain)
    }
    ...
    override fun findAllActiveByProductIds(productIds: Collection<UUID>): List<CatalogProductVariant> {
        if (productIds.isEmpty()) {
            return emptyList()
        }
        return jpaRepository.findAllByProductIdInAndIsActiveTrue(productIds).map(::toDomain)
    }
