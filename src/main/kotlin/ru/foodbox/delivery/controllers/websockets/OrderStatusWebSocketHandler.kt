...
@Component
class OrderStatusWebSocketHandler(
    private val broadcaster: OrderStatusBroadcaster,
) : TextWebSocketHandler() {
    private val log = LoggerFactory.getLogger(OrderStatusWebSocketHandler::class.java)
    ...
    override fun handleTextMessage(session: WebSocketSession, message: TextMessage) {
        ...
        when {
            payload.startsWith("subscribe") -> {
                val userId = session.attributes["user_id"] as? String ?: return
                broadcaster.subscribe(userId, session)
                session.sendMessage(PongMessage())
                log.info {"Session ${session.id} subscribed to user $userId" }
            }
            payload.startsWith("unsubscribe") -> {
                val userId = session.attributes["user_id"] as? String ?: return
                broadcaster.unsubscribe(userId, session)
                session.sendMessage(PongMessage())
                log.info {"Session ${session.id} unsubscribed from $userId" }
            }
            else -> {
